...
SanderCalculation = CalculationFactory("amber.sander")


class SanderParser(Parser):
    """
    ...
    def __init__(self, node):
        ...
        """
        super().__init__(node)
        if not issubclass(node.process_class, SanderCalculation):
            raise exceptions.ParsingError("Can only parse SanderCalculation")
